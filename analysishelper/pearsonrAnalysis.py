...
class pearsonH():
        ...
        for i in tqdm(range(test_length)):
            ...
            for j in range(i+1, test_length):
                ...
                if data_i.size == data_j.size:
                    pearson_result[i][j], pearson_p_value[i][j] = pearsonr(data_i, data_j)
    ...
    def PearsonDelay(self, start=235, delay=8, window=500):
        test_length = 100
        length = len(self.all_quotes)
        pearson_result = [[-1000 for _ in range(length)] for _ in range(length)]
        pearson_p_value = [[-1000 for _ in range(length)] for _ in range(length)]
        ...
        for i in tqdm(range(test_length)):
            ...
            for j in range(test_length):
                ...
                if i == j:
                    continue
                # pdb.set_trace()
                x,y = data_i[start:start+window], data_j[start + delay:start + delay + window]
                if x.size == y.size:
                    pearson_result[i][j], pearson_p_value[i][j] = pearsonr(x, y)
